...
try:
    if mcp_credentials:
        ...
        logger.info(f"Successfully initialized Glue client with MCP credentials for account: {mcp_credentials.get('account_id', 'unknown')}")
        ...
        try:
            ...
            actual_account = identity.get('Account', 'unknown')
            user_arn = identity.get('Arn', 'unknown')
            logger.info(f"🔐 STS VERIFICATION (DEFAULT) - Glue MCP connected to AWS Account: {actual_account}")
            logger.info(f"🔐 STS Identity ARN: {user_arn}")
        except Exception as sts_error:
