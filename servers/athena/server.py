...
try:
    if mcp_credentials:
        ...
        logger.info(f"Successfully initialized Athena clients with MCP credentials for account: {mcp_credentials.get('account_id', 'unknown')}")
        ...
        try:
            ...
            actual_account = identity.get('Account', 'unknown')
            user_arn = identity.get('Arn', 'unknown')
            logger.info(f"🔐 STS VERIFICATION (DEFAULT) - Athena MCP connected to AWS Account: {actual_account}")
            logger.info(f"🔐 STS Identity ARN: {user_arn}")
        except Exception as sts_error:
...
if __name__ == "__main__":
    try:
        ...
        if tool_count == 0:
            logger.warning("⚠️  No tools registered! This may indicate a FastMCP version compatibility issue.")
