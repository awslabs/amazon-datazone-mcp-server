...
def test_domain_id(real_datazone_client):
    ...
    if not domain_id:
        pytest.skip("TEST_DATAZONE_DOMAIN_ID environment variable not set. Integration tests require an existing domain.")
...
def tool_extractor():
    ...
    def _extract_tool(mcp_server, tool_name):
        ...
        from datazone_mcp_server.tools import (
            domain_management, project_management, data_management, 
...
class TestDomainManagementIntegration:
    ...
    @pytest.mark.asyncio
    async def test_get_domain_integration(self, real_datazone_client, test_domain_id, mcp_server_with_tools, tool_extractor):
        ...
        with patch('datazone_mcp_server.tools.domain_management.datazone_client', real_datazone_client):
    ...
    @pytest.mark.asyncio
    async def test_list_domains_integration(self, real_datazone_client, mcp_server_with_tools, tool_extractor):
    ...
    @pytest.mark.asyncio
    async def test_search_integration(self, real_datazone_client, test_domain_id, mcp_server_with_tools, tool_extractor):
...
class TestProjectManagementIntegration:
    ...
    @pytest.mark.asyncio
    async def test_list_projects_integration(self, real_datazone_client, test_domain_id, mcp_server_with_tools, tool_extractor):
        """Test listing real projects."""
        list_projects = tool_extractor(mcp_server_with_tools, 'list_projects')
        
        with patch('datazone_mcp_server.tools.project_management.datazone_client', real_datazone_client):
    ...
    @pytest.mark.asyncio
    async def test_get_project_integration(self, real_datazone_client, test_domain_id, test_project_id, mcp_server_with_tools, tool_extractor):
...
class TestDataManagementIntegration:
    ...
    @pytest.mark.asyncio
    async def test_search_listings_integration(self, real_datazone_client, test_domain_id, mcp_server_with_tools, tool_extractor):
        """Test searching real listings."""
        search_listings = tool_extractor(mcp_server_with_tools, 'search_listings')
        
        with patch('datazone_mcp_server.tools.data_management.datazone_client', real_datazone_client):
    ...
    @pytest.mark.asyncio
    async def test_list_form_types_integration(self, real_datazone_client, test_domain_id, mcp_server_with_tools, tool_extractor):
        """Test listing real form types."""
        list_form_types = tool_extractor(mcp_server_with_tools, 'list_form_types')
        
        with patch('datazone_mcp_server.tools.data_management.datazone_client', real_datazone_client):
            result = await list_form_types(
                domain_identifier=test_domain_id,
                max_results=10
            )
...
class TestErrorHandlingIntegration:
    ...
    @pytest.mark.asyncio
    async def test_nonexistent_domain_error(self, real_datazone_client, mcp_server_with_tools, tool_extractor):
        ...
        with patch('datazone_mcp_server.tools.domain_management.datazone_client', real_datazone_client):
            ...
            assert "ResourceNotFoundException" in str(exc_info.value) or "Not Found" in str(exc_info.value)

    @pytest.mark.asyncio
    async def test_invalid_parameters_error(self, real_datazone_client, test_domain_id, mcp_server_with_tools, tool_extractor):
...
class TestPerformanceIntegration:
    ...
    @pytest.mark.slow
    @pytest.mark.asyncio
    async def test_response_time_reasonable(self, real_datazone_client, test_domain_id, mcp_server_with_tools, tool_extractor):
    ...
    @pytest.mark.slow
    @pytest.mark.asyncio
    async def test_pagination_handling(self, real_datazone_client, test_domain_id, mcp_server_with_tools, tool_extractor):
