...
class TestAPIClassificationAccuracy:
    ...
    async def test_api_classification_accuracy(self):
        ...
        try:
            ...
            await self.agent.test_response(
                "ci_test_session", output_path=str(ci_results_path)
            )
            ...
            assert accuracy_results["overall_accuracy"] >= 0.7, (
                f"Overall accuracy {accuracy_results['overall_accuracy']:.2%} is below 70% threshold"
            )
            ...
            print(
                f"Correct classifications: {accuracy_results['correct_classifications']}"
            )
    ...
    def calculate_accuracy(self, df):
        ...
        correct_predictions = (
            df["api_name_normalized"] == df["api_called_normalized"]
        ).sum()
        ...
        for api in df["api_name"].unique():
            ...
            if len(api_subset) > 0:
                api_correct = (
                    api_subset["api_name_normalized"]
                    == api_subset["api_called_normalized"]
                ).sum()
    ...
    def save_results_for_ci(self, df_results, accuracy_results):
        ...
        with open(summary_path, "w") as f:
            ...
            f.write(
                f"**Overall Accuracy:** {accuracy_results['overall_accuracy']:.2%}\n\n"
            )
            ...
            f.write("\n## Test Details\n\n")
            f.write(
                "The test evaluates how accurately the AI agent can classify user questions to the correct DataZone API calls.\n"
            )
...
def test_dataset_structure():
    ...
    print(
        f"Dataset validation passed: {len(df)} questions across {df['api_name'].nunique()} API types"
    )
